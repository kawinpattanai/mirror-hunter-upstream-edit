...
from bot import DOWNLOAD_DIR, AS_DOCUMENT, AS_DOC_USERS, AS_MEDIA_USERS, CUSTOM_FILENAME, \
                 EXTENTION_FILTER, app, LEECH_LOG, BOT_PM, LEECH_LOG_ALT
                ...
                if file_.upper().endswith(VIDEO_SUFFIXES):
                    ...
                    if len(LEECH_LOG) != 0:
                        ...
                        if LEECH_LOG_ALT:
                            try:
                                for i in self.__leech_log_alt:
                                    app.send_audio(chat_id=i, audio=self.__sent_msg.audio.file_id, 
                                                   caption=cap_mono)
                            except Exception as err:
            ...
            if self.__as_doc or notMedia:
                ...
                if len(LEECH_LOG) != 0:
                    ...
                    if LEECH_LOG_ALT:
                        try:
                            for i in self.__leech_log_alt:
                                app.send_document(chat_id=i, document=self.__sent_msg.document.file_id, 
                                                  caption=cap_mono)
                        except Exception as err:
